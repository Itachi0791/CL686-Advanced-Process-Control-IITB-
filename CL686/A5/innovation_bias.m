function [X,e,U,err_est,Y,SSEST,SSESE,SSEMV] = innovation_bias()
    %clear; clc; close all;
    ...
    Q = eye(n); R_mat = 10*eye(m);
    [L_T,~,~] = dlqr(Phi', C_mat', Q, R_mat);
    L_inf = L_T';

    %% Innovation Bias
    
    K_u = C_mat*pinv(eye(n)-Phi)*Gamma_u; K_b = C_mat*pinv(eye(n)-Phi)*L_inf;
    ...
    for k=1:Ns
        ...
        % Control
        u_sp = K_u_inv*(y_sp(:,k)-K_b*e(:,k)-e(:,k));
        x_sp = inv_IminPhi*(Gamma_u*u_sp + L_inf*e(:,k));
        u(:,k) = u_sp - G_inf*(x_est(:,k)-x_sp);
        % Estimation
        x_est(:,k+1) = Phi*x_est(:,k) + Gamma_u*u(:,k) + L_inf*e(:,k);
        % Plant simulation
        U(:,k) = u(:,k) + Us;
