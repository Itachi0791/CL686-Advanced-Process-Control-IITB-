function [X,U,SSE,SSMV] = LQOC_varying()
    %clc; clear; close all;
    ...
    %% Controller Design
    Wx = eye(n); Wu = 0.1*eye(m); WN = Wx;
    S = cell(1,Ns); G = cell(1,Ns-1);
    S{Ns} = WN;
    for i = 1:Ns-1
        j = Ns-i;
        G{j} = pinv(Wu+Gamma_u'*S{j+1}*Gamma_u)*Gamma_u'*S{j+1}*Phi;
        S{j} = (Phi - Gamma_u*G{j})'*S{j+1}*(Phi - Gamma_u*G{j})+ Wx + G{j}'*Wu*G{j};
    end
   ...
   % pause(0.5)
    ...
    % sgtitle("LQOC (Time Varying) - Controlled States", "FontSize", 25, "FontWeight", "bold");
    % 
    % for i = 1:4
    %     subplot(2,2,i)
    %     hold on
    ...
    % figure;
    % set(gcf, 'WindowState', 'maximized'); 
    % sgtitle("LQOC (Time Varying) - Manipulated Inputs", "FontSize", 25, "FontWeight", "bold");
    % 
    % for i = 1:2
    %     subplot(2,1,i)
    %     hold on
