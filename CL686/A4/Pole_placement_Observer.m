function [X_hat, SSE] = Pole_placement_Observer()    
    %clear; clc; close all;
    ...
    %% Observer Design
    p_des = [0.7503;0.7655;0.6711;0.7004];
    L = (place(Phi',C_mat',p_des))';
    ...
    for k=1:Ns
        ...
        X(:,k+1) = (Z(end,:))';
        y = C_mat*(X(:,k)-Xs); % Measurement at kth instant
        % Estimate Propagation
        x_hat(:,k+1) = Phi*x_hat(:,k) + Gamma_u*u(:,k) + L*(y-C_mat*x_hat(:,k));
    end
