function [X_pred, X_est, SSE] = Kalman_Varying_Observer()    
    %clear; clc; close all;
    ...
    Q = 0.05^2*eye(n); R = 0.025^2*eye(n_op); P0 = 100*Q;
    P_pred = cell(1,Ns+1); P_pred{1} = P0; P_est = P_pred;
    L = cell(1,Ns);
    for i = 1:Ns
        P_pred{i+1} = Phi*P_est{i}*Phi' + Q; 
        L{i} = P_pred{i+1}*C_mat'*pinv(C_mat*P_pred{i+1}*C_mat' + R);
        P_est{i+1} = (eye(n)-L{i}*C_mat)*P_pred{i+1};
    end
    ...
    for k=1:Ns
        ...
        %Update Step
        x_est(:,k+1) = x_pred(:,k+1) + L{k}*(y-C_mat*x_pred(:,k+1));
    end
    X_pred = x_pred + Xs;
    X_est = x_est + Xs;
    SSE = sum((X - X_est).^2, 2);
